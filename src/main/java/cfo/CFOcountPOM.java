...
public class CFOcountPOM extends webpage
{
	...
	public static void CountGrading( ExtentTest test, String Risk) throws InterruptedException, IOException
	{
		...
		action.moveToElement(CFOcountPOM.readTotalItems1()).click().perform();		//clicking on total pages count to scroll window down
		
		Thread.sleep(1000);
		CFOcountPOM.readTotalItems1().click();					//Clicking on Text of total items just to scroll down.
		String s1 = CFOcountPOM.readTotalItems1().getText();		//Reading total number of items.
	...
	public static void CountPenalty(WebDriver driver, ExtentTest test, int valuePenalty) throws InterruptedException
	{
		...
		for(int no = 0; no < loop+1; no++)
		{
			...
			{
				Thread.sleep(100);
				CFOcountPOM.readTotalItems1().click();				//Clicking to escape unwanted tooltips
		...
		for(int no = 0; no < loop+1; no++)
		{
			...
			{
				Thread.sleep(100);
				CFOcountPOM.readTotalItems1().click();				//Clicking to escape unwanted tool tips
				CFOcountPOM.clickNextPage1().click();
			}
		}
	...
	public static void BarGraphCount(WebDriver driver, ExtentTest test, String ComplianceType, int ComplianceCount)throws InterruptedException
	{
		...
		{
			Thread.sleep(2000);
			s1 = CFOcountPOM.readTotalItems1().getText();			//Reading total number of items.
			bits = s1.split(" ");										//Splitting the String
...
WebDriverWait wait = new WebDriverWait( getDriver(), (30));
		...
		Thread.sleep(500);
		String s1 = CFOcountPOM.readTotalItems1().getText();	//Reading the total items count in String
		String[] bits = s1.split(" ");								//Splitting the String
		String itomsCount = bits[bits.length - 2];					//Getting the second last word (total number of items)
		...
		{
			Thread.sleep(1000);
			s1 = CFOcountPOM.readTotalItems1().getText();	//Reading the total items count in String
			bits = s1.split(" ");								//Splitting the String
			itomsCount = bits[bits.length - 2];					//Getting the second last word (total number of items)
		}
