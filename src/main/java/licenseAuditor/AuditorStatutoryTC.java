	...
	@Test(priority = 16)
	void BystausPendingForReviewlicense() throws InterruptedException, IOException
	{
	      test = extent.startTest("By Status-Pending For Review License Graph Working Verification");
	      LicenseCompanyadmin.StatutoryMethod.Bystatuspendingforreviewgraph( test, "Statutory");
	    	extent.endTest(test);
	        extent.flush();
	}
	@Test(priority = 17)
	...
	{
	test = extent.startTest("By License type- Shops & Commercial Establishment Pending For Review License Graph Working Verification");
	//test.log(LogStatus.INFO, "Test Initiated");
